...
extern __constant__ ContactParamsDevice c_contact;
...
// ============================================================================
__host__ __device__ inline int contactPairIndex(int a, int b, int nMaterials, int cap)
{
...
{
    return c_contact.hertzian[p * c_contact.cap + pairIdx];
}

__device__ inline double getLinearParam(const int pairIdx, const int p)
{
    return c_contact.linear[p * c_contact.cap + pairIdx];
}

__device__ inline double getBondedParam(const int pairIdx, const int p)
{
    return c_contact.bonded[p * c_contact.cap + pairIdx];
}
...
public:
                         ...
                         cudaStream_t stream)
    {
        ...
        {
            return static_cast<std::size_t>(contactPairIndex(a, b, nMat, static_cast<int>(pairTableSize)));
        };
        ...
        CUDA_CHECK(cudaMemcpyToSymbolAsync(c_contact,
                                           &dev,
