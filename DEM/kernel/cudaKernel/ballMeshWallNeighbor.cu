...
cudaStream_t stream)
{
    ...
    CUDA_CHECK(cudaMemsetAsync(sptialGrids.cellHashStart(), 0xFF, numTri * sizeof(int), stream));
    CUDA_CHECK(cudaMemsetAsync(sptialGrids.cellHashEnd(), 0xFF, numTri * sizeof(int), stream));
