...
public:
    ...
    void addWCSPHParticles(std::vector<double3> points, double3 velocity, double soundSpeed, double spacing, double density, double kinematicViscosity, cudaStream_t stream)
    {
        ...
        {
            WCSPHs_.addSPHHost(points[i], velocity, soundSpeed, mass, density, 1.3 * spacing, kinematicViscosity);
        }
    }
    ...
    void addWCSPHDummyParticles(std::vector<double3> points, double3 velocity, double soundSpeed, double spacing, double density, cudaStream_t stream)
    {
        ...
        for (size_t i = 0; i < points.size(); i++)
        {
            WCSPHs_.addDummyHost(points[i], velocity, soundSpeed, mass, density, 1.3 * spacing, 0.0);
        }
    }

    void addISPHParticles(std::vector<double3> points, double3 velocity, double spacing, double density, double kinematicViscosity, cudaStream_t stream)
    {
        if(!downloadFlag_) 
        {
            downloadFlag_ = true;
            ISPHs_.upload(stream);
        }
        double mass = spacing * spacing * spacing * density;
        for (size_t i = 0; i < points.size(); i++)
        {
            ISPHs_.addSPHHost(points[i], velocity, 0.0, mass, density, 1.3 * spacing, kinematicViscosity);
        }
    }

    void addISPHGhostParticles(std::vector<double3> points, double3 velocity, double spacing, double density, cudaStream_t stream)
    {
        if(!downloadFlag_) 
        {
            downloadFlag_ = true;
        ...
        for (size_t i = 0; i < points.size(); i++)
        {
            ISPHs_.addGhostHost(points[i], velocity, 0.0, mass, density, 1.3 * spacing, 0.0);
        }
    }

    void download(const double3 domainOrigin, const double3 domainSize, cudaStream_t stream)
    {
        if (downloadFlag_ && WCSPHs_.SPHHostSize() > 0)
        {
        ...
        else if (downloadFlag_ && ISPHs_.SPHHostSize() > 0)
        {
            ISPHs_.download(stream);
            SPHInteractions_.alloc(ISPHs_.SPHDeviceSize() * 80, stream);
            SPHInteractionMap_.alloc(ISPHs_.SPHDeviceSize() + ISPHs_.ghostDeviceSize(),
            ISPHs_.SPHDeviceSize() + ISPHs_.ghostDeviceSize(), 
            stream);
            double cellSizeOneDim = 0.0;
    ...
    void ISPH2ndIntegration(const double3 g, const double dt, const size_t gridDim_SPH, const size_t blockDim_SPH, cudaStream_t stream)
    {
        launchISPH2ndIntegration(ISPHs_,
        SPHInteractions_,
        SPHInteractionMap_,
        dt,
        gridDim_SPH,
